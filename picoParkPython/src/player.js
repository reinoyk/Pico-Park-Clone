class PlayerHandler {
    ...
    addPlayer(options) {
        // Ensure Player 2 isn't duplicated by checking the player's controls or ID
        const existingPlayer = this.game.players.find(player => player.controls === options.controls);
        if (existingPlayer) {
            return existingPlayer; // Return the existing player if already instantiated
        }
        
        // Otherwise, create and add a new player
...
class Player {
    ...
    jump(str=1) {
        if (this.ready) {
            ...
            let horizontalBoost = 3 * str * this.direction; // Gunakan this.direction
            Matter.Body.setVelocity(this.body, v(this.body.velocity.x + horizontalBoost, -15*str))
        }
    ...
    updatePlayerParts() {
        ...
        // Kurangi horizontal velocity saat di ground
        if (this.onGround()) {
            Matter.Body.setVelocity(this.body, v(
