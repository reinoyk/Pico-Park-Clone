class Renderer {
    ...
    renderPlayer(player) {
        ...
        let textX = player.body.position.x;
        let textY = player.body.position.y - (spriteSize.y * player.scale / 2) - 12; // 12px di atas kepala

        // Stroke putih biar keliatan
        let measure = this.ctx.measureText(username);
        this.ctx.strokeText(username, textX - measure.width / 2, textY);
        // Isi warna hitam/biru
        this.ctx.fillText(username, textX - measure.width / 2, textY);
